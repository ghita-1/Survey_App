...
const SurveyReport = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch survey data
        const surveyResponse = await axios.get(`http://localhost:3001/Surveys/${surveyId}`);
        ...
        <div className="chartContainerStyle">
          ...
          <div className="flex-Questions">
            {questionData.map(({ questionId, data, totalResponses }, index) => {
              const questionText = surveyQuestions[questionId] ? surveyQuestions[questionId].question : `Question ${index + 1}`;
              return (
